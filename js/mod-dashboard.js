...
const ModPayments = {
  ...
  render() {
    ...
    if (hasDates) {
      if (!this.activeYear || !years.includes(this.activeYear)) {
        ...
        // Default to the most recent month with data (i.e. the current
        // month) rather than "All Months" — All can still be picked
        // manually from the Month row.
        const monthsInYear = this._monthsForYear(all, this.activeYear);
        this.activeMonth = monthsInYear.length ? monthsInYear[monthsInYear.length - 1] : null;
      }
    }
  ...
  _renderSalesTargetChart(rows) {
    ...
    const grouped = this._groupByOwner(rows);
    const ranked = grouped.map((r) => ({
      owner: r.Owner, pct: r.target ? (r.achieved / r.target) * 100 : 0,
    })).sort((a, b) => b.pct - a.pct);
