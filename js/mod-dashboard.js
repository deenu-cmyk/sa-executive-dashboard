...
const ModDashboard = {
  ...
  render() {
    ...
    main.innerHTML = `
      ...
      <div class="chart-grid">
        <div class="chart-card span-8"><h3>Daily Leads &amp; Payments Trend</h3><div class="chart-box"><canvas id="chDailyTrend"></canvas></div></div>
        <div class="chart-card span-4"><h3>Lead Status Breakdown</h3><div class="chart-box"><canvas id="chStatusBreakdown"></canvas></div></div>
    ...
    // Bind filters and exports FIRST — guarantees they work even if a
    // chart below throws an error, instead of silently breaking everything.
    this._bindFilterBar();
    ...
    try { this._renderKPIs(rows); } catch (e) { console.error("[SA Dashboard] KPI render failed:", e); }
    try { this._renderCharts(rows); } catch (e) { console.error("[SA Dashboard] Chart render failed:", e); }
    try { this._renderInsightCarousel(rows); } catch (e) { console.error("[SA Dashboard] Carousel render failed:", e); }
    try { this._renderCalendar(rows); } catch (e) { console.error("[SA Dashboard] Calendar render failed:", e); }
  },
  ...
  _renderCharts(rows) {
    const daily = this._dailySeries(rows);
    Charts.line("chDailyTrend", daily.labels, [
      { label: "Leads", data: daily.leads },
      { label: "Payments", data: daily.payments },
    ]);

    const stageGroupsAll = Utils.groupBy(rows, "Stage");
    Charts.donut("chStatusBreakdown", Object.keys(stageGroupsAll), Object.values(stageGroupsAll).map((g) => g.length));
  ...
  _dailySeries(rows) {
    ...
    rows.forEach((r) => {
      ...
      map[d] = map[d] || { leads: 0, payments: 0 };
      map[d].leads += 1;
      map[d].payments += Utils.toNumber(r.Payment) > 0 || Utils.toNumber(r.Revenue) > 0 ? 1 : 0;
    });
    const dates = Object.keys(map).sort().slice(-30); // last 30 days with activity
    const fmt = (d) => {
      const dt = new Date(d);
    ...
    return {
      ...
      leads: dates.map((d) => map[d].leads),
      payments: dates.map((d) => map[d].payments),
    };
  },
