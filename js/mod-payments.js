...
const ModPayments = {
  ...
  render() {
    const rows = DataStore.get("paymentTrackerCSV");

    const main = Utils.qs("#mainContent");
    main.innerHTML = `
      <div class="panel">
        ${Components.sectionHead("Payment Summary by Month", "fa-calendar-days")}
        ${this._monthFilterBar(rows)}
        ${this._monthSummary(rows)}
      </div>
    ...
    Utils.qsa(".month-filter-bar .tab").forEach((btn) =>
      btn.addEventListener("click", () => { this.activeMonth = btn.dataset.month; this.render(); })
    );
  },

  // Reads the actual month out of each row's Date field — so as soon as your
  // sheet has real Date values, June/July/August etc. show up here automatically.
  // No need to maintain separate month tabs in the spreadsheet (see chat for why).
  _monthLabel(dateStr) {
  ...
  _monthFilterBar(rows) {
    const months = [...new Set(rows.map((r) => this._monthLabel(r.Date)).filter(Boolean))];
    months.sort((a, b) => new Date(`1 ${a}`) - new Date(`1 ${b}`));
    const all = ["All", ...months];
    return `<div class="tab-bar month-filter-bar">${all.map((m) => `<button class="tab ${m === this.activeMonth ? "active" : ""}" data-month="${m}">${m}</button>`).join("")}</div>`;
  },

  _monthSummary(rows) {
    const filtered = this.activeMonth === "All" ? rows : rows.filter((r) => this._monthLabel(r.Date) === this.activeMonth);
    if (!filtered.length) {
      return `<div class="empty-state"><i class="fa-solid fa-inbox"></i><p>No payment records yet${this.activeMonth === "All" ? "" : ` for ${this.activeMonth}`}.</p></div>`;
    }
    const totalAmount = Utils.sum(filtered, "Amount");
    const paidCount = filtered.filter((r) => String(r.Status).toLowerCase() === "paid").length;

    return Components.kpiRow([
      { label: "Records", value: Utils.fmtNumber(filtered.length), icon: "fa-receipt" },
      { label: "Total Amount", value: Utils.fmtCurrency(totalAmount, "USD"), icon: "fa-sack-dollar" },
      { label: "Paid", value: Utils.fmtNumber(paidCount), icon: "fa-circle-check" },
      { label: "Unpaid / Pending", value: Utils.fmtNumber(filtered.length - paidCount), icon: "fa-clock" },
    ]);
  },
