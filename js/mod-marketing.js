...
const ModMarketing = {
  ...
  // Custom calendar ranges for Week/Day mode. Each is {start, end} as
  // "YYYY-MM-DD" strings from the date pickers, or null to use the
  // auto-computed default (this period vs previous period).
  period1: null, // "Compare Filter 1" — the current/primary period
  period2: null, // "Compare Filter 2" — the period to compare against
  ...
  render() {
    ...
    if (this.compareGranularity === "month") {
      ...
      // Week/Day mode ignores the Month dropdown entirely and compares
      // calendar periods using each row's actual Date value. Defaults are
      // anchored to the latest date found in the data (not the real system
      // date, so this still works with historical/sample datasets), and
      // either side can be overridden with an explicit date range via the
      // "Compare Filter 1" / "Compare Filter 2" pickers.
      const filteredAll = this._applyFilters(all);
      const period = this._resolvePeriods(this.compareGranularity, filteredAll);
    ...
    main.innerHTML = `
      <div class="filter-bar">
        ...
        ${this.compareGranularity !== "month" ? `
        <div class="filter-group">
          <label>Compare Filter 1</label>
          <div class="date-range-inputs" style="display:flex;align-items:center;gap:6px;">
            <input type="date" id="fPeriod1Start" value="${this.period1?.start || ""}" />
            <span style="opacity:.6;">to</span>
            <input type="date" id="fPeriod1End" value="${this.period1?.end || ""}" />
          </div>
        </div>
        <div class="filter-group">
          <label>Compare Filter 2</label>
          <div class="date-range-inputs" style="display:flex;align-items:center;gap:6px;">
            <input type="date" id="fPeriod2Start" value="${this.period2?.start || ""}" />
            <span style="opacity:.6;">to</span>
            <input type="date" id="fPeriod2End" value="${this.period2?.end || ""}" />
          </div>
        </div>` : ""}
  ...
  // Picks the latest date actually present in the (filtered) data as the
  // anchor for "today", so Week/Day defaults always land on real rows
  // instead of silently showing zeros when the dataset is historical or
  // doesn't reach the real calendar date.
  _latestDataDate(rows) {
    const dates = rows.map((r) => this._dateOnly(r.Date)).filter(Boolean);
    return dates.length ? new Date(Math.max(...dates)) : this._dateOnly(new Date());
  },

  // Combines the auto-computed default period with any explicit override
  // from the "Compare Filter 1" / "Compare Filter 2" date pickers.
  _resolvePeriods(granularity, rows) {
    const anchor = this._latestDataDate(rows);
    const defaults = this._periodRanges(granularity, anchor);

    const hasRange = (p) => p && p.start && p.end;
    const toRange = (p) => ({ start: this._dateOnly(p.start), end: this._dateOnly(p.end) });
    const fmtRange = (r) => `${r.start.toLocaleDateString("en-US", { month: "short", day: "numeric" })} – ${r.end.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;

    const current = hasRange(this.period1) ? toRange(this.period1) : defaults.current;
    const previous = hasRange(this.period2) ? toRange(this.period2) : defaults.previous;

    return {
      current,
      previous,
      currentLabel: hasRange(this.period1) ? fmtRange(current) : defaults.currentLabel,
      previousLabel: hasRange(this.period2) ? fmtRange(previous) : defaults.previousLabel,
    };
  },

  _periodRanges(granularity, anchorDate) {
    const today = anchorDate || this._dateOnly(new Date());
  ...
  _bindFilterBar() {
    Utils.qsa("#fGranularity button").forEach((btn) => btn.addEventListener("click", () => {
      this.compareGranularity = btn.dataset.granularity;
      // Switching granularity clears any custom ranges so the new mode
      // starts from its own sensible default instead of a stale range.
      this.period1 = null;
      this.period2 = null;
      this.render();
    }));
    ...
    Utils.qs("#fReset").addEventListener("click", () => { this.filters = { program: "", country: "" }; this.compareMonth = undefined; this.period1 = null; this.period2 = null; this.render(); });
    ...
    // Compare Filter 1 / 2 — only present in Week/Day mode. Both start and
    // end must be picked before the custom range takes effect; until then
    // the auto-computed default (anchored to the latest data date) is used.
    const p1s = Utils.qs("#fPeriod1Start"), p1e = Utils.qs("#fPeriod1End");
    const p2s = Utils.qs("#fPeriod2Start"), p2e = Utils.qs("#fPeriod2End");
    const updatePeriod1 = () => { this.period1 = { start: p1s.value, end: p1e.value }; this.render(); };
    const updatePeriod2 = () => { this.period2 = { start: p2s.value, end: p2e.value }; this.render(); };
    if (p1s) p1s.addEventListener("change", updatePeriod1);
    if (p1e) p1e.addEventListener("change", updatePeriod1);
    if (p2s) p2s.addEventListener("change", updatePeriod2);
    if (p2e) p2e.addEventListener("change", updatePeriod2);
  },
