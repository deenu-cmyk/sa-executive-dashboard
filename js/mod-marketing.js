...
const ModMarketing = {
      ...
      <div class="chart-grid">
        <div class="chart-card span-6">
          <div class="chart-card-head">
            <h3>Spend by Program · ${this._monthLabel(this.activeMonth)}</h3>
        ...
        <div class="chart-card span-6"><h3>ROAS vs CPL — Efficiency Scatter</h3><p class="chart-sub">Bubble size = spend · ${this._monthLabel(this.activeMonth)}</p><div class="chart-box"><canvas id="chEfficiency"></canvas></div></div>
        <div class="chart-card span-6"><h3>Spend Breakdown · ${this._monthLabel(this.activeMonth)}</h3><div id="rankedSpend" class="ranked-bar-list"></div></div>
      </div>

      <div class="panel">
        ${Components.sectionHead(`Program Performance Detail — ${this._monthLabel(this.activeMonth)}`, "fa-table-list")}
  ...
  _buildInsight(programs, monthly, stats) {
    ...
    const parts = [];
    parts.push(`<b>${this._monthLabel(this.activeMonth)}${isBreakout ? " — breakout month" : ""}:</b> ROAS hit <b>${stats.roas.toFixed(2)}x</b> (${Utils.fmtCurrency(stats.revenue, "INR")} revenue vs ${Utils.fmtCurrency(stats.spend, "INR")} spend).`);
