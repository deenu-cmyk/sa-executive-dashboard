...
function toISODate(value) {
  ...
  if (typeof value === "string") {
    ...
    // DD/MM/YYYY (with optional time after a space)
...
const MockData = (() => {
  ...
  function paymentRows() {
    const regions = ["India", "USA", "APAC", "EMEA", "Chat"];
    const rows = [];
    for (let i = 0; i < 500; i++) {
  ...
  function salesTargetRows() {
    const regions = ["USA", "India", "Chat", "EMEA", "APAC"];
    const rows = [];
    regions.forEach((region) => {
      for (let i = 0; i < rand(2, 6); i++) {
        const target = rand(5000, 45000);
        rows.push({
          Region: region,
          Owner: pick(owners),
          Target: target,
  ...
  const generators = {
    ...
    salesTargetCSV: salesTargetRows,
  };
